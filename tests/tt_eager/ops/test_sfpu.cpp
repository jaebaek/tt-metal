    ...
    for(const std::string& sfpu_op_name : sfpu_op) {
        ...
        if ( tt::tt_metal::is_parametrized_type(unary_op_type) ) {
            if (unary_op_type == tt::tt_metal::UnaryOpType::EXP) {
                sfpu_op_to_hlk_op_name[sfpu_op_name]  = eltwise_unary_op_utils::get_block_defines({tt::tt_metal::UnaryWithParams{unary_op_type, 1.0}});
            } else {
                sfpu_op_to_hlk_op_name[sfpu_op_name]  = eltwise_unary_op_utils::get_block_defines({tt::tt_metal::UnaryWithParams{unary_op_type, 0.5}});
            }
        } else {
            sfpu_op_to_hlk_op_name[sfpu_op_name]  = eltwise_unary_op_utils::get_block_defines({tt::tt_metal::UnaryWithParams{unary_op_type}});
        }
    }
}
