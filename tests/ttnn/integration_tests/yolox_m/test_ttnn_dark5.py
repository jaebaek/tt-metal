...
def test_dark5(device, reset_seeds, model_location_generator):
    ...
    ds_state_dict = {k: v for k, v in state_dict.items() if (k.startswith(("backbone.backbone.dark5")))}
    ...
    torch_input_tensor = torch.randn(1, 384, 40, 40)  # Batch size of 1, 128 input channels, 160x160 height and width
    ...
    output_tensor = output_tensor.reshape(1, 20, 20, 768)
    output_tensor = torch.permute(output_tensor, (0, 3, 1, 2))
    output_tensor = output_tensor.to(torch_input_tensor.dtype)
    assert_with_pcc(torch_output_tensor, output_tensor, pcc=0.99)  # PCC = 0.9564151119600169
