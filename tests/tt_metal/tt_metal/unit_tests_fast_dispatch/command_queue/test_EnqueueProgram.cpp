...
TEST_F(CommandQueueMultiDeviceFixture, TestRandomizedProgram) {
    ...
    for (const auto &test_device: this->devices_) {
        ...
        log_info(tt::LogTest, "Running random programs test on device {}.", test_device->id());
        ...
        for (uint32_t i = 0; i < NUM_PROGRAMS; i++) {
            ...
            tt::tt_metal::detail::CompileProgram(test_device, program);
        }

        log_info(tt::LogTest, "Running {} programs for cache warmup.", programs.size());
        // This loop caches program and runs
        for (Program& program: programs) {
            EnqueueProgram(test_device->command_queue(), program, false);
        }
        ...
        Finish(test_device->command_queue());
    }
}
