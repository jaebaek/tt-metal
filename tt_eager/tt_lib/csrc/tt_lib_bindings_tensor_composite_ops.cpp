...
void TensorModuleCompositeOPs(py::module& m_tensor) {
    ...
    m_tensor.def(
        "lti",
        py::overload_cast<Tensor&, Tensor&, std::optional<MemoryConfig>>(&lti),
        py::arg("input_a").noconvert(),
        py::arg("input_b").noconvert(),
        py::arg("output_mem_config").noconvert() = operation::DEFAULT_OUTPUT_MEMORY_CONFIG,
        R"doc(
    ...
    m_tensor.def(
        "lti",
        py::overload_cast<Tensor&, float, std::optional<MemoryConfig>>(&lti),
        py::arg("input_a").noconvert(),
        py::arg("value").noconvert(),
        py::arg("output_mem_config").noconvert() = operation::DEFAULT_OUTPUT_MEMORY_CONFIG,
        R"doc(
            Performs element-wise lesser than inplace comparison on tensor ``input_a`` and a scalar ``value``.
