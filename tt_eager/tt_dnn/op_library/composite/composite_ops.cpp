...
Tensor _lti(Tensor& input_a, Tensor& input_b, std::optional<MemoryConfig> output_mem_config) {
    lt(input_a, input_b, std::nullopt, operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::nullopt, input_a);
    return input_a;
}
Tensor lti(Tensor& input_a, Tensor& input_b, std::optional<MemoryConfig> output_mem_config) {
    return operation::decorate_as_composite(__func__, _lti)(input_a, input_b, output_mem_config);
}

Tensor _lti_overload(Tensor& input_a, float value, std::optional<MemoryConfig> output_mem_config) {
    Tensor input_b = full_like(input_a, value);
    lt(input_a, input_b, std::nullopt, operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::nullopt, input_a);
    return input_a;
}
Tensor lti(Tensor& input_a, float value, std::optional<MemoryConfig> output_mem_config) {
    return operation::decorate_as_composite(__func__, _lti_overload)(input_a, value, output_mem_config);
}
