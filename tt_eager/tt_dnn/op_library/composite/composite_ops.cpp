...
Tensor _gti(Tensor& input_a, Tensor& input_b, std::optional<MemoryConfig> output_mem_config) {
    gt(input_a, input_b, std::nullopt, operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::nullopt, input_a);
    return input_a;
}
Tensor gti(Tensor& input_a, Tensor& input_b, std::optional<MemoryConfig> output_mem_config) {
    return operation::decorate_as_composite(__func__, _gti)(input_a, input_b, output_mem_config);
}

Tensor _gti_overload(Tensor& input_a, float value, std::optional<MemoryConfig> output_mem_config) {
    Tensor input_b = full_like(input_a, value);
    gt(input_a, input_b, std::nullopt, operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::nullopt, input_a);
    return input_a;;
}
Tensor gti(Tensor& input_a, float value, std::optional<MemoryConfig> output_mem_config) {
    return operation::decorate_as_composite(__func__, _gti_overload)(input_a, value, output_mem_config);
}
