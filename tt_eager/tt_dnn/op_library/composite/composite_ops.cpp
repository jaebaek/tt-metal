    ...
    return where(
        ...
        (value == 0) ? t_nan
                     : where(
                           ...
                           t_nan,
                           mul(t_inf, sign(input_b, output_mem_config), std::nullopt, output_mem_config),
                           output_mem_config),
        result,
        output_mem_config);
}
...
Tensor _lei(Tensor& input_a, Tensor& input_b, std::optional<MemoryConfig> output_mem_config) {
    lte(input_a, input_b, std::nullopt, operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::nullopt, input_a);
    return input_a;
}
Tensor lei(Tensor& input_a, Tensor& input_b, std::optional<MemoryConfig> output_mem_config) {
    return operation::decorate_as_composite(__func__, _lei)(input_a, input_b, output_mem_config);
}

Tensor _lei_overload(Tensor& input_a, float value, std::optional<MemoryConfig> output_mem_config) {
    Tensor input_b = full_like(input_a, value);
    lte(input_a, input_b, std::nullopt, operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::nullopt, input_a);
    return input_a;
}
Tensor lei(Tensor& input_a, float value, std::optional<MemoryConfig> output_mem_config) {
    return operation::decorate_as_composite(__func__, _lei_overload)(input_a, value, output_mem_config);
}
...
Tensor _logit(const Tensor& input_a, float eps, const MemoryConfig& output_mem_config) {
    ...
    Tensor logit_input = where(
        ...
        input_a,
        where(
            lt(input_a, t_eps, std::nullopt, output_mem_config),
            t_eps,
            where(gt(input_a, t1m_eps, std::nullopt, output_mem_config), t1m_eps, input_a, output_mem_config),
            output_mem_config),
        output_mem_config);
    t_eps.deallocate();
    t1m_eps.deallocate();
