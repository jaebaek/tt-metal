...
void GroupAttnMatmul::validate(const std::vector<Tensor>& input_tensors) const {
    ...
    if (input_tensor_a.is_sharded()) {
        ...
        TT_FATAL(input_tensor_a.num_cores() == ashape[1], "Q heads must be sharded on number of q heads!");
    ...
    if (input_tensor_b.is_sharded()) {
        ...
        TT_FATAL(input_tensor_b.num_cores() == bshape[0], "KV heads must be sharded on batch!");
