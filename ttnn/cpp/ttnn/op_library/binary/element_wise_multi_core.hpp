...
namespace binary {
namespace element_wise_multi_core {
...
inline auto create(const auto& operation_attributes, const auto& tensor_args, auto& tensor_return) {
    ...
    if (operation_attributes.activations.has_value()) {
        const auto activations_as_strings = operation_attributes.activations.value();
        std::transform(
            activations_as_strings.begin(),
            activations_as_strings.end(),
            std::back_inserter(fused_activations),
    ...
    std::map<string, string> eltwise_defines =
        eltwise_binary_op_utils::get_defines(op_type, output.get_dtype(), fused_activations);
