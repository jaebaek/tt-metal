...
struct ShardSpec {
    ...
    const uint32_t num_max_cores() const {return this->grid.num_cores();}
   ...
   public:
    ...
    uint32_t num_cores() const{
        if(!is_sharded(this->buffer_layout_))
            ...
            uint32_t total_height = this->shard_spec().tensor2d_shape[0] * this->shard_spec().page_shape[0];
            uint32_t total_width = this->shard_spec().tensor2d_shape[1] * this->shard_spec().page_shape[1];
            return get_num_cores_from_shape(this->buffer_layout(), total_height, total_width, this->shard_spec().shape());
        }
    }
