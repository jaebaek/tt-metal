...
void Device::compile_command_queue_programs() {
    ...
    if (this->is_mmio_capable()) {
        ...
        for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
            ...
            tt_cxy_pair prefetch_core =
                dispatch_core_manager::get(num_hw_cqs).prefetcher_core(device_id, channel, cq_id);
            tt_cxy_pair dispatch_core =
                dispatch_core_manager::get(num_hw_cqs).dispatcher_core(device_id, channel, cq_id);
            ...
            tt::tt_metal::CreateSemaphore(
                *command_queue_program_ptr, prefetch_core, 0, dispatch_core_type);  // prefetch_h_exec_buf_sem
        ...
        TT_ASSERT(
            prefetch_core.chip == mmio_device_id and dispatch_core.chip == mmio_device_id,
            "Prefetcher is on device {} and Dispatcher is on device {} but they are expected to be on device {}",
            prefetch_core.chip,
            dispatch_core.chip,
            mmio_device_id);
        ...
        log_debug(
            LogDevice,
            "Prefetch H logical location: {} physical core: {}",
            prefetch_core.str(),
            prefetch_physical_core.str());
        log_debug(
            LogDevice,
            "Dispatch H logical location: {} physical core {}",
            dispatch_core.str(),
            dispatch_physical_core.str());
        ...
        TT_ASSERT(
            tunneler_location.chip == mmio_device_id,
            "Tunneler is on device {} but it is expected to be on device {}",
            tunneler_location.chip,
            mmio_device_id);
        CoreCoord r_tunneler_core = std::get<1>(tt::Cluster::instance().get_connected_ethernet_core(
            std::make_tuple(tunneler_location.chip, tunneler_core)));
        ...
        tt::tt_metal::CreateSemaphore(
            *mmio_command_queue_program_ptr, prefetch_core, 0, dispatch_core_type);  // prefetch_sync_sem
        tt::tt_metal::CreateSemaphore(
            *mmio_command_queue_program_ptr,
            ...
            dispatch_core_type);  // prefetch_sem
        tt::tt_metal::CreateSemaphore(
            *mmio_command_queue_program_ptr, prefetch_core, 0, dispatch_core_type);  // prefetch_h_exec_buf_sem
        ...
        tt_metal::CreateSemaphore(
            *mmio_command_queue_program_ptr, dispatch_core, 0, dispatch_core_type);  // dispatch_sem
        ...
        std::vector<uint32_t> mux_compile_args = {
            ...
            packet_switch_4B_pack(
                (uint32_t)prefetch_physical_core.x,
        ...
        std::vector<uint32_t> demux_compile_args = {
            ...
            ((1 << dispatch_constants::DISPATCH_BUFFER_LOG_PAGE_SIZE) *
             dispatch_constants::get(dispatch_core_type).dispatch_buffer_pages()) >>
                4,                                              // 9: remote_tx_queue_size_words 0
        ...
        tt_cxy_pair prefetch_d_core =
            dispatch_core_manager::get(num_hw_cqs).prefetcher_d_core(device_id, channel, cq_id);
        ...
        tt::tt_metal::CreateSemaphore(
            *command_queue_program_ptr, prefetch_d_core, 0, dispatch_core_type);  // prefetch_d_sync_sem
        tt::tt_metal::CreateSemaphore(
            *command_queue_program_ptr, prefetch_d_core, 0, dispatch_core_type);  // prefetch_d_upstream_cb_sem
        tt::tt_metal::CreateSemaphore(
            *command_queue_program_ptr,
            prefetch_d_core,
            dispatch_buffer_pages,
            dispatch_core_type);  // prefetch_d_downstream_cb_sem
        ...
        tt::tt_metal::CreateSemaphore(
            *command_queue_program_ptr,
            dispatch_core,
            dispatch_buffer_pages,
            dispatch_core_type);  // dispatch_downstream_cb_sem
        ...
        std::vector<uint32_t> demux_d_compile_args = {
            ...
            packet_switch_4B_pack(
                ...
                (uint32_t)DispatchRemoteNetworkType::NOC0),   // 7: remote_tx_3_info
            (dispatch_constants::DISPATCH_BUFFER_BASE >> 4),  // 8: remote_tx_queue_start_addr_words 0
            dispatch_constants::get(dispatch_core_type).prefetch_d_buffer_size() >>
                4,                                              // 9: remote_tx_queue_size_words 0
            0,                                                  // 10: remote_tx_queue_start_addr_words 1
            0,                                                  // 11: remote_tx_queue_size_words 1
            0,                                                  // 12: remote_tx_queue_start_addr_words 2
        ...
        uint32_t scratch_db_base =
            (dispatch_constants::DISPATCH_BUFFER_BASE +
             dispatch_constants::get(dispatch_core_type).prefetch_d_buffer_size() + PCIE_ALIGNMENT - 1) &
            (~(PCIE_ALIGNMENT - 1));
        ...
        std::vector<uint32_t> mux_d_compile_args = {
            ...
            1,                            // 3: mux_fan_in
            packet_switch_4B_pack(
                (uint32_t)dispatch_physical_core.x,
...
void Device::configure_command_queue_programs() {
    ...
    for (uint8_t cq_id = 0; cq_id < this->num_hw_cqs(); cq_id++) {
        ...
        tt::Cluster::instance().write_sysmem(
            pointers.data(), pointers.size() * sizeof(uint32_t), cq_id * cq_size, mmio_device_id, channel);
    }
    ...
    for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
        ...
        if (device_id != mmio_device_id) {
            tt_cxy_pair dispatch_d_location =
                dispatch_core_manager::get(num_hw_cqs).dispatcher_d_core(device_id, channel, cq_id);
            dispatch_core_type = dispatch_core_manager::get(num_hw_cqs).get_dispatch_core_type(device_id);
...
void Device::initialize_command_queue() {
    TT_ASSERT(
        this->is_mmio_capable() or (not this->is_mmio_capable() and this->num_hw_cqs() == 1),
        "Only support one hardware command queue for fast dispatch on remote device");
        ...
        for (const auto &[core_type, logical_dispatch_cores] : command_queue_program.logical_cores()) {
            for (const CoreCoord &logical_dispatch_core : logical_dispatch_cores) {
                ...
                tt::llrt::write_launch_msg_to_core(
                    this->id(), this->physical_core_from_logical_core(logical_dispatch_core, core_type), &msg);
            }
        }
    }
    ...
    if (!this->is_mmio_capable()) {
        ...
        for (const auto &[core_type, logical_dispatch_cores] : mmio_command_queue_program.logical_cores()) {
            for (const CoreCoord &logical_dispatch_core : logical_dispatch_cores) {
                launch_msg_t msg =
                    mmio_command_queue_program.kernels_on_core(logical_dispatch_core, core_type)->launch_msg;
                tt::llrt::write_launch_msg_to_core(
                    mmio_device_id,
...
bool Device::initialize(size_t l1_small_size, const std::vector<uint32_t> &l1_bank_remap, bool minimal) {
    ...
    log_info(tt::LogMetal, "Initialized cluster");
    this->initialize_allocator(l1_small_size, l1_bank_remap);
    log_info(tt::LogMetal, "Initialized allocator");
    this->initialize_build();
    log_info(tt::LogMetal, "Initialized build");
    ...
    if (!already_initialized) {
        log_info(tt::LogMetal, "Building FW");
        this->build_firmware();
        log_info(tt::LogMetal, "Built FW");
    }

    DprintServerAttach(this);
    watcher_init(this);

    log_info(tt::LogMetal, "Launching FW");
    this->initialize_and_launch_firmware();
    log_info(tt::LogMetal, "Launched FW");
        ...
        for (const chip_id_t &device_id : tt::Cluster::instance().get_devices_controlled_by_mmio_device(this->id_)) {
            ...
            for (uint8_t cq_id = 0; cq_id < curr_num_hw_cqs; cq_id++) {
                if (device_id == this->id_) {
                    // mmio device.
                    if (dispatch_core_manager::get(curr_num_hw_cqs)
                            .is_dispatcher_core_allocated(device_id, curr_channel, cq_id)) {
                        tt_cxy_pair dispatch_location =
                            dispatch_core_manager::get(curr_num_hw_cqs).dispatcher_core(device_id, curr_channel, cq_id);
                    ...
                    if (dispatch_core_manager::get(curr_num_hw_cqs)
                            .is_prefetcher_core_allocated(device_id, curr_channel, cq_id)) {
                        tt_cxy_pair prefetch_location =
                            dispatch_core_manager::get(curr_num_hw_cqs).prefetcher_core(device_id, curr_channel, cq_id);
                    ...
                    if (dispatch_core_manager::get(curr_num_hw_cqs)
                            .is_mux_core_allocated(device_id, curr_channel, cq_id)) {
                        tt_cxy_pair mux_location =
                            dispatch_core_manager::get(curr_num_hw_cqs).mux_core(device_id, curr_channel, cq_id);
                    ...
                    if (dispatch_core_manager::get(curr_num_hw_cqs)
                            .is_demux_core_allocated(device_id, curr_channel, cq_id)) {
                        tt_cxy_pair demux_location =
                            dispatch_core_manager::get(curr_num_hw_cqs).demux_core(device_id, curr_channel, cq_id);
...
CoreCoord Device::compute_with_storage_grid_size() const { return tt::get_compute_grid_size(id_, num_hw_cqs_); }
...
void Device::begin_trace(const uint8_t cq_id, const uint32_t tid, const uint32_t trace_buff_size) {
    ...
    detail::EnableAllocs(this);
    this->trace_buffer_pool_.insert(
        {tid, Trace::create_trace_buffer(this->command_queue(cq_id), desc, trace_buff_size)});
...
void Device::end_trace(const uint8_t cq_id, const uint32_t tid) {
    TT_FATAL(
        ...
        tid);
    TT_FATAL(
        this->trace_buffer_pool_.count(tid) > 0, "Trace instance " + std::to_string(tid) + " must exist on device");
