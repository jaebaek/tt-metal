...
void init_compute_and_storage_l1_bank_manager(Allocator &allocator, const AllocatorConfig &alloc_config) {
    ...
    for (uint32_t y = 0; y < alloc_config.worker_grid_size.y; y++) {
        for (uint32_t x = 0; x < alloc_config.worker_grid_size.x; x++) {
            ...
            if (alloc_config.core_type_from_noc_coord_table.at(noc_core) == AllocCoreType::ComputeAndStore) {
                ...
                for (int storage_bank_index = 0; storage_bank_index < num_banks.per_storage_core;
                     storage_bank_index++) {
                    uint32_t remapped_bank_id = shuffled_bank_id[bank_id];
                    ...
                    if (alloc_config.l1_bank_size != alloc_config.worker_l1_size) {
                        ...
                        bank_offset_bytes =
                            static_cast<int64_t>(storage_core_offset) -
                            alloc_config.l1_bank_size;  // Assuming top-down here --  Not sure if this is hacky... need
    ...
    uint64_t interleaved_address_limit =
        static_cast<uint64_t>(alloc_config.worker_l1_size - alloc_config.l1_bank_size) + STORAGE_ONLY_UNRESERVED_BASE;
    ...
    // Assuming top down allocation for L1 buffers so the allocatable memory space is the top alloc_config.l1_bank_size
    // bytes of L1
    uint64_t alloc_offset = L1_UNRESERVED_BASE;
    allocator.l1_manager = BankManager(
        BufferType::L1, bank_id_to_bank_offset, allocatable_l1_size, interleaved_address_limit, alloc_offset);
