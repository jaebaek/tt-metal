...
def pytest_runtest_makereport(item, call):
    ...
    # call_report = item.stash.get(phase_report_key, {}).get('call')
    ...
    #     if hasattr(item, '_test_failed'):
    #         logger.debug('Test failed. Resetting with smi')
    # run_process_and_get_result("tt-smi -tr all", {}, False)


@pytest.hookimpl(hookwrapper=True)
def pytest_runtest_teardown(item, nextitem):
    yield
    pipeline_type = item.config.getoption("--pipeline-type")
    valid_pipelines = ["models_device_performance_bare_metal"]
    if pipeline_type in valid_pipelines:
        logger.debug(f"Running custom teardown")
        report = item.stash[phase_report_key]
        test_failed = report.get("call", None) and report["call"].failed
        if test_failed:
            arch = item.config.getoption("--tt-arch")
            check_smi = run_process_and_get_result("tt-smi-metal")
            # logger.debug(f"check smi: {check_smi.returncode}")
            if check_smi.returncode > 0:
                logger.debug(f"Test failed - resetting {arch} with tt-smi")
                if arch == "grayskull":
                    smi_reset_result = run_process_and_get_result("tt-smi -tr all")
                elif arch == "wormhole_b0":
                    smi_reset_result = run_process_and_get_result("tt-smi -wr all")
                else:
                    raise Exception(f"Unrecognized arch for tensix-reset: {arch}")
            else:
                logger.debug(f"Test failed - resetting {arch} with tt-smi-metal script")
            ...
            logger.debug(f"tt-smi reset status: {smi_reset_result.returncode}")
